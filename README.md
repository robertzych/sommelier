...
- **Retrieval quality** — q015 is a known reranker miss: the expected source reaches candidates@20 at rank 1 but is dropped by the cross-encoder. Candidates: `rerank_top_k=7` as a zero-cost mitigation, Cohere Rerank, and larger cross-encoder models (`ms-marco-MiniLM-L-12-v2`) or table-aware rerankers that may score tabular content more accurately.
