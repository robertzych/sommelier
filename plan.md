...
### Retrieval Quality
1. Alternative Rankers Fix (addresses q015): q015 is a reranker miss — README.md reaches candidates@20 at rank 1 but is dropped by the cross-encoder. Test `rerank_top_k=7` as a zero-cost mitigation (rank 1 survives a looser cutoff); test Cohere Rerank API (`reranker = "cohere"`) against the golden set; evaluate larger cross-encoder models (e.g., `ms-marco-MiniLM-L-12-v2`, `ms-marco-MiniLM-L-6-v2`) and table-aware rerankers — table-dense docs (broker.md, schema.md) may benefit from rerankers that score tabular content more accurately; measure HR@5 and MRR@5 before/after each change and choose the configuration that improves q015 without regressing other questions.

### Evaluations
1. Expanded evaluation matrix: add Haiku-without-RAG and Sonnet-with-RAG as two new baselines in `golden_set.json`; run all 25 questions through each and score manually using the same 3-point rubric; compare whether retrieval augmentation narrows or widens the quality gap between model tiers (Haiku vs Sonnet) — the delta measures how much of Sommelier's quality advantage comes from retrieval vs model strength.
2. Cross-document benchmark questions: add 5–10 questions to `golden_set.json` requiring synthesis across multiple documents or chunks (e.g., "How does upsert behavior differ for realtime vs offline tables?"); set `expected_sources` to all required files; use Full Recall Rate as the primary metric for these questions since partial retrieval will produce an incomplete answer.
3. Multi-rater evaluation: score a 20% sample of sommelier answers independently with a second rater; compute inter-rater agreement (Cohen's κ) on `completeness`, which is the most subjective dimension; use disagreements to tighten the rubric or add concrete examples to the JUDGE_PROMPT before enabling automated scoring.

### Ingestion
1. Annotation pipeline optimization: (a) batch LLM annotation requests — group all code-heavy chunks from a single file and send in one LiteLLM call to reduce per-file annotation time; (b) add content-hash annotation caching — store `(chunk_hash → annotation)` in a sidecar SQLite so annotations survive re-ingestion without re-querying the LLM; (c) evaluate smaller/local annotation models (e.g., `ollama/llama3.1:8b`) as a zero-cost fallback with acceptable annotation quality; (d) selective annotation — skip annotation for chunks whose BM25 sparse score already exceeds a threshold at retrieval time, since those chunks already surface on keyword signal alone.
